...
- [x] Uploaded the latest codebase to GitHub, including new utility modules (`log_config.py`, `verify_exchanges.py`) and staging all modified core scripts. (2026-03-18)
- [x] Consolidated dual stop-loss execution logic: Removed `check_holdings_monitor` from `etf_screening.py` to adhere strictly to DRY principles. Portfolio monitoring and stop-loss execution are now exclusively the responsibility of `etf_monitoring.py`.
- [x] Added Gemini API 3-attempt retry with exponential backoff (2s, 4s) in `select_etfs_with_gemini()` in `etf_screening.py`. After all retries fail, falls back to `_fallback_top7()`.
- [x] Added comprehensive pytest-based unit test suite (`tests/` directory, 48 tests). Covers `calculate_rsi`, `calculate_metrics`, `db_manager` (fill_slot, clear_slot, trigger_stop_loss, reconcile, portfolio metrics), `kis_api` (mocked SDK), `select_etfs_with_gemini` (mocked Gemini with retry tests), and `common.py` (mocked calendar). All external APIs (KIS, Gemini, Telegram, FinanceDataReader, yfinance) are mocked in `conftest.py`. Created `requirements-test.txt` for CI.
- [x] Added GitHub Actions CI pipeline (`.github/workflows/ci.yml`) that runs `pytest` on push/PR to `main` with Python 3.11.
